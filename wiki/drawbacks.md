...
# Drawbacks
 ...
 - Blocks introduced by a *keyword* **CAN'T** be *broken*. You cannot use neither `return`, `goto`, `break` or `continue` to jump out of any `try`, `catch`, `finally`, `with` or `use` blocks. You can `throw` exceptions, though. The same apply to blocks introduced by `e4c_using_context` or `e4c_reusing_context`.
