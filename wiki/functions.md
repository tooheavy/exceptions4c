...
![](https://raw.githubusercontent.com/guillermocalvo/exceptions4c/master/etc/img/icons/24x24/exception_handling.png) **Exception context handling functions**:
...
![](https://raw.githubusercontent.com/guillermocalvo/exceptions4c/master/etc/img/icons/24x24/beautifully_crafted.png) **Other integration and convenience functions**:
...
- [e4c_using_context](functions.md#e4c_using_context)
- [e4c_reusing_context](functions.md#e4c_reusing_context)

----

# ![][FUNCTION] `e4c_context_begin`
...
This function begins the current exception context to be used by the program (or current thread), until [e4c_context_end](functions.md#e4c_context_end) is called.
...
The convenience function [e4c_print_exception](functions.md#e4c_print_exception) will be used as the default [uncaught handler](types.md#e4c_uncaught_handler). It will be called in the event of an uncaught exception, before exiting the program or thread. This handler can be set through the function [e4c_context_set_handlers](functions.md#e4c_context_set_handlers).
...
# ![][FUNCTION] `e4c_context_set_signal_mappings`
...
- `mappings` **must** be terminated by [E4C_NULL_SIGNAL_MAPPING](macros.md#E4C_NULL_SIGNAL_MAPPING).
...
# ![][FUNCTION] `e4c_context_get_signal_mappings`
...
The function [e4c_is_instance_of](functions.md#e4c_is_instance_of) will determine if the thrown exception is an instance of any of the defined exception types. The type of the thrown exception can also be compared for an exact type match.
...
## See Also
 ...
 - [e4c_exception](types#e4c_exception)
...
This macro is intended to be used in a `catch` block, or in a `finally` block provided that some exception was actually thrown (i.e. [e4c_get_status](functions.md#e4c_get_status) returned `e4c_failed` or `e4c_recovered`).
...
This is a convenience function for showing an error message through the standard error output. It can be passed to [e4c_context_set_handlers](functions.md#e4c_context_set_handlers) as the handler for uncaught exceptions, and will be used, by default, unless otherwise set up.
...
# ![][FUNCTION] `e4c_print_exception_type`
...
[FUNCTION]:
https://raw.githubusercontent.com/guillermocalvo/exceptions4c/master/etc/img/icons/40x40/function.png
